...
function getSumBetweenNumbers(n1, n2) {
  const arrVal = Array.from({ length: n2 - n1 + 1 }, (_, index) => n1 + index);
  const res = arrVal.reduce((acc, item) => acc + item, 0);
  return res;
}
...
function reverseInteger(num) {
  const reversedStringInteger = String(num).split('').reverse().join('');
  return parseInt(reversedStringInteger, 10);
}
...
function evaluateTicTacToePosition(position) {
  const checkWin = (a, b, c) => a === b && b === c && a !== undefined;

  for (let i = 0; i < 3; i += 1) {
    if (checkWin(position[i][0], position[i][1], position[i][2])) {
      return position[i][0];
    }
  }

  for (let i = 0; i < 3; i += 1) {
    if (checkWin(position[0][i], position[1][i], position[2][i])) {
      return position[0][i];
    }
  }

  if (checkWin(position[0][0], position[1][1], position[2][2])) {
    return position[0][0];
  }
  if (checkWin(position[0][2], position[1][1], position[2][0])) {
    return position[0][2];
  }
